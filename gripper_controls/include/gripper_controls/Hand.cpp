...
bool Hand::slide_left_up(gripper_controls::PositionCommand::Request &req, gripper_controls::PositionCommand::Response &res){
  ...
  send_torque = command_torque(0, -0.3);
  if (send_torque){
    float initial_position;
...
// Testing:
bool Hand::command_testing(gripper_controls::PositionCommand::Request &req, gripper_controls::PositionCommand::Response &res){
  // std::cout << "Service empty" << std::endl;
  bool L{1};
  set_friction_left(L);
  return 1;
}
